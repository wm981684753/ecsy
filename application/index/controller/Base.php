...
class Base extends Controller
{
    ...
    {
        $uid = session('user_id');
        $uinfo = db('xy_users')->field('deal_status,status,balance,deal_count,deal_time,deal_reward_count dc')->find($uid);
		$user_level = db('xy_users')
        ->alias('u')
        ->leftJoin('xy_user_level ul','ul.id= u.level_id')
        ->field('ul.name,ul.order_num')
        ->where('u.id',session('user_id'))
        ->find();
		$user_level_deal = $user_level['order_num'] ? $user_level['order_num'] : 0;
        ...
        if($uinfo['deal_time']==strtotime(date('Y-m-d'))){
            ...
            if($uinfo['deal_count'] >= $user_level_deal+$uinfo['dc']) return ['code'=>1,'info'=>'今日交易次数已达上限!'];
        }else{
            //重置最后交易时间
