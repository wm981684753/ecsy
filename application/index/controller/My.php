...
class My extends Base
{
    ...
    {
        $this->info = db('xy_users')->field('username,tel,id,headpic,balance,freeze_balance,invite_code,show_td,level_id')->find(session('user_id'));
        $this->lv3 = [0,config('vip_3_num')];
        $this->lv2 = [0,config('vip_2_num')];
        $level_name = db('xy_user_level')->where('id',$this->info['level_id'])->field('name')->find();
		$this->level_name = $level_name['name'];
        return $this->fetch();
    }
