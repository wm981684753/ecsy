...
class Order extends Base
{
   /**
	...
	public function save_order(){
		...
		if(request()->isPost()){
            $oid = input('post.oid/s','');
			$con = Db::name('xy_convey')->where('id',$oid)->find();
			if($con){
				Db::startTrans();
				try{
					//var_dump(config('order_freeze'));die;
					Db::name('xy_convey')->where('id',$oid)->update(['status'=>5,'endtime'=>time()+config('order_freeze')]);
					Db::name('xy_users')->where('id',$con['uid'])->update(['deal_status' => 1,'status'=>1]);
					Db::name('xy_users')->where('id',$con['uid'])->setDec('balance',$con['num']);
					Db::name('xy_users')->where('id',$con['uid'])->setInc('freeze_balance',$con['num']);
					Db::commit();    
	...
	public function jieshou(){
		$id=input("post.id");
		$uid = session('user_id');
		$list=db('xy_convey')->where('id',$id)->find();
		$a=5;
		$jine=$list['num'];
		
		$aa=db('xy_convey')->where('id',$id)->setField('status',$a);
		$aaa=db('xy_users')->where('id',$uid)->find();
		$where['balance']=$aaa['balance']+$jine;
		
		$where['freeze_balance']=$aaa['freeze_balance']-$jine;
		var_dump($where);exit();
		$abc=db('xy_users')->where('id',$uid)->update($where);
		return json(['code'=>0,'info'=>'验证通过']);
	}
}