...
class Index extends Controller
{
    ...
    public function main()
    {
        ...
        $this->sell_num = db('xy_convey')->where('status',1)->where('endtime','between',[strtotime(date('Y-m-d')),time()])->sum('num');
        $this->sell_y_num = db('xy_convey')->where('status',1)->where('endtime','between',[strtotime(date('Y-m-d'))-24*3600,strtotime(date('Y-m-d'))])->sum('num');
        $this->sell_count = db('xy_convey')->where('status',1)->where('endtime','between',[strtotime(date('Y-m-d')),time()])->count('id');
        $this->sell_y_count = db('xy_convey')->where('status',1)->where('endtime','between',[strtotime(date('Y-m-d'))-24*3600,strtotime(date('Y-m-d'))])->count('id');
        $this->new_user = db('xy_users')->where('status',1)->where('addtime','between',[strtotime(date('Y-m-d')),time()])->count('id');
        $this->new_y_user = db('xy_users')->where('status',1)->where('addtime','between',[strtotime(date('Y-m-d'))-24*3600,strtotime(date('Y-m-d'))])->count('id');
        $this->user_order = db('xy_convey')->where('status',1)->where('addtime','between',[strtotime(date('Y-m-d')),time()])->field('uid')->Distinct(true)->select();
        $this->user_order = count($this->user_order);
        $this->user_y_order = db('xy_convey')->where('status',1)->where('addtime','between',[strtotime(date('Y-m-d'))-24*3600,strtotime(date('Y-m-d'))])->Distinct(true)->select();
